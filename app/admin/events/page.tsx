...
import { Calendar, MapPin, Star, Ticket, Image as ImageIcon } from 'lucide-react';
...
export default async function AdminEventsPage() {
  // 🛠 THE FIX: We now count ticketBatches instead of listings!
  const events = await prisma.event.findMany({
    ...
    include: {
      _count: {
        select: { ticketBatches: true } // Fixed from 'listings'
      }
    }
  });
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-950 border-b border-zinc-800 text-xs font-black uppercase tracking-widest text-zinc-500">
                  ...
                  <th className="p-6 text-right">Featured Status</th>
                </tr>
              </thead>
