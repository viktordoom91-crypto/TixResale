...
import { Save, ArrowLeft, Ticket, MapPin, Calendar, Image as ImageIcon, FileText, DollarSign, Trash2, Shapes } from 'lucide-react';
...
async function updateEvent(formData: FormData) {
  ...
  await prisma.event.update({
    ...
    data: {
      ...
      description,
      city,
      category, // Saved to DB so filters work correctly
...
export default async function EditEventPage({ params }: { params: { id: string } }) {
  ...
  // Format the Date object for the HTML datetime-local input (captures date AND time)
      ...
      <header className="bg-zinc-950 border-b border-zinc-900 py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <form action={deleteEvent} onSubmit={(e) => { if(!confirm('Are you sure you want to permanently delete this event?')) e.preventDefault(); }}>
            ...
            <button type="submit" className="flex items-center justify-center w-full sm:w-auto gap-2 px-4 py-2 bg-red-500/10 text-red-500 border border-red-500/20 rounded-xl hover:bg-red-500 hover:text-white transition-all text-xs font-black uppercase tracking-widest">
          ...
          <form action={updateEvent} className="space-y-8 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 flex items-center gap-2"><Shapes className="w-3.5 h-3.5 text-lime-400" /> Event Category</label>
                <select name="category" defaultValue={(event as any).category || 'Concerts'} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-white font-medium focus:outline-none focus:border-lime-500 focus:ring-1 focus:ring-lime-500 transition-all">
                  <option value="Concerts">Artist Tour / Concert</option>
              ...
              <div className="space-y-2 md:col-span-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 flex items-center gap-2"><FileText className="w-3.5 h-3.5 text-lime-400" /> Venue & Description</label>
                <textarea name="description" defaultValue={event.description || ''} rows={4} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-white font-medium focus:outline-none focus:border-lime-500 focus:ring-1 focus:ring-lime-500 transition-all placeholder-zinc-700 resize-none" placeholder="e.g. Main Stage at Madison Square Garden. VIP doors open at 7PM." />
              </div>
